...
class AnomalyScore(Base):
    ...
    )  # statistical, rule_based, ml_based
    algorithm_name = Column(
        String(100), nullable=False
    )  # zscore, isolation_forest, etc.
    ...
    # Additional metadata
    algorithm_metadata = Column(JSONB, nullable=True)  # Algorithm-specific data
    ...
    __table_args__ = (
        Index(
            "idx_anomaly_scores_transaction_analysis",
            "transaction_id",
            "analysis_run_id",
        ),
...
class RuleFlag(Base):
    ...
    rule_metadata = Column(JSONB, nullable=True)  # Rule-specific metadata
    ...
    __table_args__ = (
        Index(
            "idx_rule_flags_transaction_analysis", "transaction_id", "analysis_run_id"
        ),
