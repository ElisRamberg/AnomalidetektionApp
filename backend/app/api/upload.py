...
from ..schemas.upload import (
    FileUploadResponse, FileUploadStatus, FileUploadListResponse,
    FileUploadStats
)
...
async def get_upload_history(
    ...
    count_result = await db.execute(
        select(FileUpload.id).where(query.whereclause if query.whereclause is not None else True)
    )
...
async def get_upload_stats(db: AsyncSession = Depends(get_async_db)):
    ...
    processing_uploads = len([u for u in uploads if u.status in ["uploaded", "processing"]])
