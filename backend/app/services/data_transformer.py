...
class DataTransformerService:
    ...
    def transform_transactions(
        self, raw_transactions: List[Dict[str, Any]], upload_id: str
    ) -> pd.DataFrame:
        """
    ...
    def _ensure_transaction_ids(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "id" not in df.columns or df["id"].isnull().any():
            ...
            df["id"] = df.apply(
                lambda row: (
                    str(uuid.uuid4()) if pd.isnull(row.get("id")) else row.get("id")
                ),
                axis=1,
            )
    ...
    def _validate_required_fields(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if df.empty:
            raise DataTransformationError(
                "No valid transactions after cleaning required fields"
            )
    ...
    def _standardize_timestamps(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if null_timestamps.any():
                print(
                    f"Warning: {null_timestamps.sum()} timestamps could not be parsed"
                )
    ...
    def _clean_amounts(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if null_amounts.any():
                print(
                    f"Warning: {null_amounts.sum()} invalid amounts found and will be dropped"
                )
                df = df[~null_amounts]
            ...
            if settings.max_transaction_amount:
                ...
                if large_amounts.any():
                    print(
                        f"Warning: {large_amounts.sum()} transactions exceed maximum amount threshold"
                    )
    ...
    def _standardize_account_ids(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            raise DataTransformationError(
                f"Account ID standardization failed: {str(e)}"
            )
    ...
    def _add_derived_fields(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            df["time_since_prev_hours"] = (
                df["time_since_prev"].dt.total_seconds() / 3600
            )
        ...
        try:
            ...
            if "_source_file" in df.columns:
                ...
                df["raw_data"] = df.apply(
                    lambda row: {
                        "original_data": {
                            k: v for k, v in row.items() if k not in ["raw_data"]
                        }
                    },
                    axis=1,
                )
    ...
    def get_transformation_stats(
        self, original_count: int, final_df: pd.DataFrame
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "drop_rate": (
                (original_count - len(final_df)) / original_count
                if original_count > 0
                else 0
            ),
            "date_range": {
                "start": (
                    final_df["timestamp"].min().isoformat()
                    if not final_df.empty
                    else None
                ),
                "end": (
                    final_df["timestamp"].max().isoformat()
                    if not final_df.empty
                    else None
                ),
            },
            "amount_stats": {
                ...
                "mean": (
                    float(final_df["amount"].mean()) if not final_df.empty else None
                ),
                "total": (
                    float(final_df["amount"].sum()) if not final_df.empty else None
                ),
            },
            "account_count": (
                final_df["account_id"].nunique() if not final_df.empty else 0
            ),
            "features_added": [
                "year",
                "month",
